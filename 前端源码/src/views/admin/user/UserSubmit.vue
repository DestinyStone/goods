    ...
    methods: {
      ...
      submitHandler() {
        ...
        if (this.checkRegisterAccount && this.checkRegisterPassword && this.checkRegisterTel && this.checkRegisterEmail) {
          ...
          if (user.userId) {
            User.update(user.userId, user).then(() => {
      ...
      init() {
        ...
        if (this.id) {
          User.getById(this.id).then(res => {
            res = res.data.data;
            $("#user-submit-account").val(res.account);
            $("#user-submit-password").val(res.password);
            $("#user-submit-tel").val(res.tel);
            $("#user-submit-email").val(res.email);
