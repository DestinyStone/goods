<template>
  <div>
        ...
        <table id="table" class="table table-striped" style="text-align: center">
          <tbody>
            ...
            <tr v-for="item in data" :key="item.id">
              ...
              <td>
                <button @click="openUpdate(item.id)" class="btn btn-info" type="submit">
                  <span class="glyphicon glyphicon-pencil"></span>
                </button>
              </td>
              <td>
                <button @click="openDelete(item.id)" class="btn btn-info" type="submit">
    ...
    <group-submit :id="currentId" v-if="showSubmit" @close="() => {showSubmit = false; this.onLoad()}"/>
...
<script>
  ...
  import GroupSubmit from "./group/GroupSubmit";

  export default {
    name: "GroupManager",
    components: {GroupSubmit, CommonDelete},
    data() {
      return {
        data: [],
        showSubmit: false,
    ...
    methods: {
      ...
      handlerDelete() {
        Group.delete(this.currentId).then(() => {
